...
document.addEventListener('showAlert', function (type) {
  var icons = {
  ...
  el.innerHTML = "\n    <div x-data=\"{ open: false }\" x-init=\"() => { setTimeout(() => { open = true }, 100); setTimeout(() => { open = false }, 3000) }\" class=\"fixed flex items-center justify-center top-0 right-0 w-full z-50\">\n      <div class=\"w-full max-w-sm m-2 py-3 px-4 overflow-hidden shadow-lg rounded-md flex items-center border ".concat(shades[type.detail.type], "\"\n        x-show=\"open\"\n        x-transition:enter=\"transition ease-out duration-300\"\n        x-transition:enter-start=\"opacity-0 transform -translate-y-5\"\n        x-transition:enter-end=\"opacity-100 transform translate-y-0\"\n        x-transition:leave=\"transition ease-in duration-300\"\n        x-transition:leave-start=\"opacity-100 transform translate-y-0\"\n        x-transition:leave-end=\"opacity-0 transform -translate-y-5\"\n      >\n      ").concat(icons[type.detail.type], "\n        <div class=\"ml-4 flex-1\">\n            <div class=\"text-lg text-gray-600 font-semibold\">").concat(document.querySelector(".language-helper .".concat(type.detail.type)).innerText, "</div>\n            <div class=\"text-sm\">").concat(type.detail.message, "</div>\n        </div>\n        <div x-on:click=\"open = false\" class=\"ml-4 text-gray-500 w-5 cursor-pointer hover:text-gray-900\">\n            <svg xmlns=\"http://www.w3.org/2000/svg\" fill=\"none\" viewBox=\"0 0 24 24\" stroke=\"currentColor\">\n                <path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M6 18L18 6M6 6l12 12\" />\n            </svg>\n        </div>\n      </div>\n    </div>\n    ");
  setTimeout(function () {
    return document.body.append(el);
  }, 100);
});
document.addEventListener('showNewMailNotification', function (message) {
  notifyUser(message.detail.subject, "".concat(message.detail.sender_name, " - ").concat(message.detail.sender_email));
});
function notifyUser() {
...
(function () {
  ...
  function resizeCaptcha() {
    if (document.querySelector(".captcha")) {
      var y = 0;
      var x = document.getElementById("user").offsetWidth;
      ...
      var scale = x / y;
      document.querySelector(".captcha").style.transform = "scale(".concat(scale, ", 1)");
  ...
  document.querySelectorAll(".btn_copy").forEach(function (el) {
  ...
  function copyEmailAddress(element) {
    ...
    if (email) {
      ...
      el.remove();
      var event = new CustomEvent("showAlert", {
        bubbles: true,
        detail: {
          type: "success",
  ...
  document.getElementById("email_id") && document.getElementById("email_id").addEventListener("click", function () {
    document.getElementById("email_id").disabled = true;
  });

  //Scrolling to Div
  document.querySelector(".messages") && document.querySelector(".messages").addEventListener("click", function () {
    setTimeout(function () {
      scroll({
  ...
  //Cookie Policy
  document.getElementById("cookie") && document.addEventListener("DOMContentLoaded", function () {
  ...
  //Cookie Policy Close
  document.getElementById("cookie_close") && document.getElementById("cookie_close").addEventListener("click", function () {
  ...
  //Locale Update
  document.getElementById("locale") && document.getElementById("locale").addEventListener("change", function (e) {
    var form = document.getElementById("locale-form");
    form.action = form.action + "/".concat(e.target.value);
    form.submit();
  });

  //Locale Update
  document.getElementById("locale-mobile") && document.getElementById("locale-mobile").addEventListener("change", function (e) {
    var form = document.getElementById("locale-form-mobile");
    form.action = form.action + "/".concat(e.target.value);
    form.submit();
  });

  //Download Email
  document.addEventListener("loadDownload", function () {
    document.querySelectorAll(".download").forEach(function (el) {
      el.addEventListener("click", function (e) {
        e.preventDefault();
        if (document.querySelector("#email-".concat(e.target.dataset.id)) === null) {
          var a = document.createElement("a");
          a.id = "email-".concat(e.target.dataset.id);
          a.download = "email-".concat(e.target.dataset.id, ".eml");
  ...
  function makeTextFile(id) {
    var textFile = null;
    text = document.querySelector("#message-".concat(id, " textarea")).value;
      ...
      blogs: function blogs() {
        ...
        fetch(fetchUrl).then(function (response) {
          ...
          blogs.forEach(function (item) {
            data += "\n              <a class=\"bg-gray-50 rounded-lg shadow-sm\" href=\"".concat(item.link, "\" target=\"_blank\">\n                <article class=\"relative\">\n                  <img class=\"rounded-t-lg h-60 w-full object-cover\" src=\"").concat(item._embedded["wp:featuredmedia"] ? item._embedded["wp:featuredmedia"][0].media_details.sizes.medium.source_url : "", "\" alt=\"featured_image\">\n                  <span class=\"absolute top-5 left-5 bg-black text-white px-3 py-1 rounded-full text-sm\">").concat(item._embedded["wp:term"][0][0].name, "</span>\n                  <div class=\"p-4 xl:p-6 2xl:p-8 space-y-2\">\n                      <h4 class=\"font-semibold text-lg\">").concat(item.title.rendered.replace(/(<([^>]+)>)/gi, ""), "</h4>\n                      <p class=\"text-sm text-gray-600\">").concat(item.excerpt.rendered.replace(/(<([^>]+)>)/gi, ""), "</p>\n                  </div>\n                </article>\n              </a>\n              ");
          });
          data += "</section>";
          if (blogs.length) {
...
/******/ 	__webpack_require__.O(undefined, ["css/vendor","css/common"], () => (__webpack_require__("./resources/js/app.js")))
/******/ 	__webpack_require__.O(undefined, ["css/vendor","css/common"], () => (__webpack_require__("./resources/css/common.scss")))
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, ["css/vendor","css/common"], () => (__webpack_require__("./resources/css/vendor.scss")))
